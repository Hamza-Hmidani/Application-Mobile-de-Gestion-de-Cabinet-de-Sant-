...
public class ConsultationActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        afficherInfosPatientRoom(patientId);
    ...
    // Récupère le profil du patient depuis Room
    private void afficherInfosPatientRoom(int patientId) {
        new Thread(() -> {
            com.example.cabinetmedical.Entity.user patient = com.example.cabinetmedical.dao.AppDatabase.getInstance(this).userDao().getUserById(patientId);
            runOnUiThread(() -> {
                if (patient != null) {
                    tvPatientNom.setText("👤 " + patient.getPrenom() + " " + patient.getNom());
                    tvPatientEmail.setText("✉️ " + patient.getEmail());
    ...
    private LinearLayout buildConsultationCard(int id, String desc, String date, double prix, String medecinName) {
        ...
        tvId.setTextColor(getResources().getColor(R.color.primary_red));
        tvId.setTextSize(13);
        ...
        tvPrice.setTextColor(getResources().getColor(R.color.primary_red));
